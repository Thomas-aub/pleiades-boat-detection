...
def rotated_iou(pts_a: np.ndarray, pts_b: np.ndarray) -> float:
    ...
    Uses Shapely to safely handle massive float64 CRS coordinates 
...
def build_size_dataframe(
    labels_root: Path,
    images_root: Path,   # 🔥 NEW
    splits: Sequence[str],
    class_names: Dict[int, str],
) -> pd.DataFrame:
    """
    Version FULL-SCENE compatible (no tiles).
    Uses real image dimensions instead of tile_size.
    """
    ...
    for split in splits:
        ...
        if not split_dir.exists():
            logger.warning("'%s' not found, skipped.", split_dir)
            continue

        for txt in split_dir.glob("*.txt"):
            # 🔥 Find corresponding image
            img_path = img_dir / (txt.stem + ".tif")
            if not img_path.exists():
                logger.warning("Image not found for %s", txt.name)
                continue
            ...
            for line in txt.read_text().splitlines():
                ...
                # ✅ Correct denormalisation
                pts_px = [
                    (coords[2 * k] * img_w, coords[2 * k + 1] * img_h)
                    for k in range(4)
                ]
