...
def _poly_area(pts: np.ndarray) -> float:
    ...
    Args:
        pts: Array of shape ``(N, 2)`` — polygon vertices in order.

    Returns:
        Unsigned area as a float.
    """
    x, y = pts[:, 0], pts[:, 1]
...
def rotated_iou_crs(pts_a: np.ndarray, pts_b: np.ndarray) -> float:
    ...
    Uses ``cv2.intersectConvexConvex`` for the intersection, then derives
    union from individual areas minus overlap.
    ...
    inter_area = _poly_area(inter.reshape(-1, 2))
    area_a     = _poly_area(pts_a)
    area_b     = _poly_area(pts_b)
